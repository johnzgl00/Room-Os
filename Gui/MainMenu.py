...
import HandTrackingMain as ht
import FaceTrackingMain as ft
...
def OpenMenu():
	...
	piNum.place(relx=0.1 ,rely=0.35)
        ...
        #seperator
	separator = ttk.Separator(window, orient='vertical')
	separator.place(relx=0.18, rely=0.05, relwidth=0.001, relheight=1)
	
	#HandTracking
	htrack = tk.Button(window, text="HandTracking", font=("Arial", 15), bg='grey88', command=ht.handTrack)
	htrack.place(relx=0.3, rely=0.1)
	#FaceTracking
	ftrack = tk.Button(window, text="FaceTracking", font=("Arial", 15), bg='grey88', command=ft.faceTrack)
